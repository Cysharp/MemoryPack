...
export class Sub {
    ...
    static serialize(value: Sub | null): Uint8Array {
        const writer = MemoryPackWriter.getSharedInstance();
        this.serializeCore(writer, value);
        return writer.toArray();
    }

    static serializeCore(writer: MemoryPackWriter, value: Sub | null): void {
        if (value == null) {
    ...
    static deserialize(buffer: ArrayBuffer): Sub | null {
        return this.deserializeCore(new MemoryPackReader(buffer));
    }

    static deserializeCore(reader: MemoryPackReader): Sub | null {
