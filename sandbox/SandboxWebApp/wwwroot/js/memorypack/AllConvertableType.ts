...
export class AllConvertableType {
    ...
    static deserializeCore(reader: MemoryPackReader): AllConvertableType | null {
        ...
        if (count == 43) {
            ...
            value.myDictionary = reader.readMap(reader => reader.readInt32(), reader => reader.readInt32(), true); if (count == 36) return value;
            value.mySet = reader.readSet(reader => reader.readInt32()); if (count == 37) return value;
            value.myNestedNested = reader.readArray(reader => reader.readMap(reader => reader.readInt32(), reader => reader.readSet(reader => reader.readArray(reader => reader.readString())), false)); if (count == 38) return value;
            value.dictCheck2 = reader.readMap(reader => reader.readUint8(), reader => reader.readBoolean(), true); if (count == 39) return value;
            value.dictCheck3 = reader.readMap(reader => reader.readGuid(), reader => reader.readNullableInt32(), true); if (count == 40) return value;
            value.dictCheck4X = reader.readMap(reader => reader.readInt32(), reader => reader.readString(), false); if (count == 41) return value;
